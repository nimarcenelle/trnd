 ...
 * watchouts, and concrete first moves. Deterministic template here; the model
...
export function buildFallbackBrief(business: Business, services: Service[]): NewBusinessBrief {
  ...
  // (deeper, with lexicon and subreddits) once the model runs.
...
export async function generateBusinessBrief(
  ...
  if (isModelConfigured) {
    try {
      const { generateBriefWithModel } = await import("./openai");
      const text = siteText ?? (await fetchSiteTextForBrief(business.website));
      return await generateBriefWithModel(business, services, text);
    } catch (err) {
      console.warn("[ai] model brief failed — using deterministic fallback:", (err as Error).message);
    }
  }
