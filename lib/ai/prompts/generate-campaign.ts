...
const ANGLE_FIELDS = (ctx: PromptCtx) => [
  ...
  "  price when there is one) or unmistakably clear terms. Ten words at most —",
  "  it is set over the photo in the ad, where a sentence is unreadable.",
  "  \"$8 Driade Shake, weekday afternoons\", not a description of the drink.",
...
export function generateAssetsPrompt(
  ...
  return [
    ...
    "- headlines: exactly 5, each under 40 characters. Shorter wins — five",
    "  words that land beat ten that get clipped.",
    "- primary_texts: exactly 3 paid-social primary texts, 1-2 sentences and",
    "  under 125 characters each. The first sentence must carry the whole",
    "  offer on its own, because most people never expand the rest.",
