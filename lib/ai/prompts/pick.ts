...
export function buildPickPrompt(ctx: PickPromptCtx): string {
  ...
  return [
    ...
    `- scripts: exactly 3 short-form video scripts, each on a different thesis. Pick the three that fit this term, for example: problem first (open on the problem the term names), the objection (open on the one doubt that stops them buying and answer it), the specific trigger (the moment the term happens in someone's day), before and after (only when the guardrail allows it for this item), the item in use in one take, the switch from what they use now (never name a rival). Never a price-led script: the price is never the hook or the thesis, it belongs in the close only. Each has:`,
