...
export function buildPickPrompt(ctx: PickPromptCtx): string {
  ...
  return [
    ...
    "",
    "Return JSON:",
    `- finding: the mismatch between the customer's words and the brand's words, in exactly this shape: Your customers are searching "${term}." Your ${page} says "<the item's name or the brand's own words for it>." Quote the term exactly. No numbers, no percentages, no advice.`,
    ...
    `  - beats: ${3} to ${5} beats in order. visual: what the phone is pointed at, shootable by one person. on_screen_text: six words at most, or "" when the shot carries it. vo: the spoken line, or "".`,
    `  - cta: the last line, naming the item${matchedPrice ? " and its listed price" : ""}.`,
