 ...
 * (lib/ai/gemini.ts), which reports usage on every response; this file is
...
export function recordAiUsage(model: string, usage: UsageMetadata | undefined | null): void {
  const input = Math.max(0, Math.round(usage?.promptTokenCount ?? 0));
  const output = Math.max(0, Math.round(usage?.candidatesTokenCount ?? 0));
