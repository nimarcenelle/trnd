...
function askFor(kind: RefinementKind, note: string): string {
  switch (kind) {
    case "footage":
...
export async function rotateHook(brief: CreativeBrief): Promise<CreativeBrief | null> {
...
export async function refinePickAction(_prev: RefineState, formData: FormData): Promise<RefineState> {
  ...
  if (kind === "hook" && !isGeminiConfigured) {
    const rotated = await rotateHook(detail.pick.brief);
