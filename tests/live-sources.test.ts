...
describe("liveSources", () => {
  it("names what runs keyless, and lists the rest as waiting on a key", () => {
    ...
    expect(liveSourcesLine(s)).toBe("Google Trends, Autocomplete, TikTok trending board");
  ...
  it("claims YouTube and TikTok profiles only when their keys are set", () => {
    ...
    expect(liveSourcesLine(s)).toBe("Search volume by metro, Google Trends, Autocomplete, YouTube Shorts, TikTok trending board, TikTok profiles, Reddit, Rival Meta ads");
